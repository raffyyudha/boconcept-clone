      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-[#5f605f] max-w-2xl mx-auto">
            Contact {whatsappName} (林祥龙) to receive a free quote, order fabric samples, or book an on-site balcony measurement.
          </p>
        </div>

        <div className="grid md:grid-cols-3 gap-8">
          {/* WhatsApp Card */}
          <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="group relative overflow-hidden flex flex-col justify-between p-8 bg-[#14100b] text-[#f4f4f4] h-64 border border-[#3e3d3a]/30 hover:border-[#d4af37] transition-all">
            <div>
              ...
              <p className="text-xs text-[#8b8c8b] mt-2">{whatsappName} (林祥龙)<br/>+65 {whatsappNumber.replace('65', '').replace(/(\d{4})(\d{4})/, '$1 $2')}</p>
            </div>
            <p className="text-sm underline hover:no-underline text-white mt-4">Open Chat Directly</p>
          </a>

          {/* Website QR Card */}
          <a href={websiteUrl} target="_blank" rel="noopener noreferrer" className="group relative overflow-hidden flex items-center justify-between p-8 bg-[#14100b] text-[#f4f4f4] h-64 border border-[#3e3d3a]/30 hover:border-[#d4af37] transition-all">
            ...
            <div className="w-24 h-24 sm:w-28 sm:h-28 bg-white p-1.5 rounded-none flex items-center justify-center shrink-0 border border-neutral-800">
              <img src={websiteQr} alt="Oscar Furnishing Website QR Code" className="w-full h-full object-contain" />
            </div>
          </a>

          {/* Email / Social */}
          <a href={`mailto:${email}`} className="group relative overflow-hidden flex flex-col justify-between p-8 bg-[#14100b] text-[#f4f4f4] h-64 border border-[#3e3d3a]/30 hover:border-[#d4af37] transition-all">
            <div>
              ...
              <div className="space-y-1 mt-3 text-xs text-[#8b8c8b]">
                <p>Email: <span className="text-white hover:underline">{email}</span></p>
                <p>Facebook: <span className="text-white hover:underline">{facebookUrl}</span></p>
              </div>
            </div>
