...
export default function HeroSection({ data, whatsappNumber = "6591229599" }: HeroSectionProps) {
  const subtitle = data?.subtitle || "YOUR IDEAL CHOICE FOR HOME FURNISHINGS";
  const title = data?.title || "Bespoke home solutions that elevate your space";
  const bgImage = data?.background_image || "https://ext.same-assets.com/2889591500/1674530610.jpeg";
  const waText = data?.whatsapp_text || "WhatsApp Alvin Lim: +65 9122 9599";
  const ctaText = data?.cta_link_text || "Get a free quote and consultation";
